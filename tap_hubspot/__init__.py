...
def load_schema(entity_name):
    ...
    schema = utils.load_json(get_abs_path(
        'schemas/{}.json'.format(entity_class)))
...
def _sync_contact_vids(catalog, vids, schema, bumble_bee, stream_name="contacts"):
    ...
    for record in data.values():
        ...
        singer.write_record(stream_name, record, catalog.get(
...
def sync_contact_list_contacts(STATE, ctx):
    ...
    singer.write_schema(stream_id, schema, ["vid"], [
                        bookmark_key], catalog.get('stream_alias'))
...
def add_property_params_from_state(state, params):
    # for new_param in state['params']:
    #     if new_param['properties']:
    #         for property in new_param['properties']:
    #             params['properties'].append(property)

    merged_properties = set(params['properties'] + state['properties'])
    params['properties'] = list(merged_properties)
...
def populate_streams_from_catalog(catalog):
    for stream in catalog.get('streams'):
        ...
        if (stream_id.startswith('contact_list:')):
            ...
            stream = Stream(stream_id, sync_contact_list_contacts, [
                            "vid"], 'versionTimestamp', 'FULL_TABLE')
